...
#Messy data- lots of NaN values and mismatched data types
```
...
#This should plot ratings on x and imdb ratings on y. Showing correlation
#IMDB users are pretty mid lol.
```
...
#bin=true made the x axis discrete. It seems that they were all collapsed to 5, 15, 25, etc.
```
...
#bin=alt.BinParams(maxbins=20) lets us say custom bins
```
...
#It arranged the genres alphabetically even though we made the data nominal. What gives??
```
...
#Hey, I remember this from stats class lol.
```
...
```{python}
movies_gross = movies[['US Gross', 'Release Date']].sort_values(by="US Gross", ascending=False)
movies_gross.head()
#Need to sort this by gross
```
...
#Note the use of Mark Area 
#We aggregate by adding the aggregator before the variable in parentheses
```
